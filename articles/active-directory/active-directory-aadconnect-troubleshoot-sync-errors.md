...
Following section describes different types of synchronization errors that can occur during the export operation to Azure AD using the Azure AD connector. This connector can be identified by the name format being "contoso.*partner.onmschina.cn*".
...
1. Update the user's UserPrincipalName in Azure AD from bob@contoso.com to bob@contoso.partner.onmschina.cn. You can use the following PowerShell command with the Azure AD PowerShell Module:
