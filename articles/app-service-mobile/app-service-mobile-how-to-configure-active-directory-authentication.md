...
3. Sign in to the [Azure Classic Management Portal] and navigate to **Active Directory**.
...
8. Once the application has been added, click the **Configure** tab. Edit the **Reply URL** under **Single Sign-on** to be the URL of your application appended with the path, _/.auth/login/aad/callback_. For example, `https://contoso.azurewebsites.cn/.auth/login/aad/callback`. Make sure that you are using the HTTPS scheme.
...
11. Within the root **EntityDescriptor** element, there should be an **entityID** attribute of the form `https://sts.chinacloudapi.cn/` followed by a GUID specific to your tenant (called a "tenant ID"). Copy this value - it will serve as your **Issuer URL**. You will configure your application to use this later.
