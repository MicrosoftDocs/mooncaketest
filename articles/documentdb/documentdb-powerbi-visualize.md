---
title: Power BI tutorial for DocumentDB connector | Azure
...
- Access to our demo account or data in your Azure DocumentDB account.
  ...
  - Or, to create your own account, see [Create a DocumentDB database account using the Azure portal](./documentdb-create-account.md). Then, to get sample volcano data that's similar to what's used in this tutorial (but does not contain the GeoJSON blocks), see the [NOAA site](https://www.ngdc.noaa.gov/nndc/struts/form?t=102557&s=5&d=5) and then import the data using the [DocumentDB data migration tool](./documentdb-import-data.md).
...
5. Click on **Azure**, select **Azure DocumentDB (Beta)**, and then click **Connect**.  The **Azure DocumentDB Connect** window should appear.
...
- To learn more about DocumentDB, see the [DocumentDB documentation landing page](./index.md).