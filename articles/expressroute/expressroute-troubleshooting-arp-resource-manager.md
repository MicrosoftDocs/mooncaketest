...
在继续下一步之前，请确保你已具备以下条件
 ...
 - 用于配置对等互连（Azure 专用和 Azure 公共）的 IP 地址范围。查看 [ExpressRoute 路由要求页](./expressroute-routing.md)中的 IP 地址分配示例，了解如何将 IP 地址映射到你所在的一侧和 ExpressRoute 侧的接口。你可以通过查看 [ExpressRoute 对等互连配置页](./expressroute-howto-routing-arm.md)了解对等互连配置。
