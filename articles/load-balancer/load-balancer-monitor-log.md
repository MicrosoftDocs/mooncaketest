...
* **审核日志：**可以使用 [Azure 审核日志](../monitoring-and-diagnostics/insights-debugging-with-events.md)（以前称为操作日志）查看提交到你的 Azure 订阅的所有操作及其状态。默认情况下，审核日志已启用，并可在 Azure 门户中查看。
...
登录到 [Azure 门户](http://portal.azure.cn)。如果你还没有负载均衡器，请先[创建负载均衡器](./load-balancer-get-started-internet-arm-ps.md)，然后继续。
...
* **Azure 工具：**通过 Azure PowerShell、Azure 命令行界面 (CLI)、Azure REST API 或 Azure 门户检索审计日志中的信息。[使用资源管理器审核操作](../azure-resource-manager/resource-group-audit.md)一文中详细介绍了每种方法的分步说明。
