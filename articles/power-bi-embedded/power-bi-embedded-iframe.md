---
title: How to use Power BI Embedded with REST | Azure
...
An overview of Power BI Embedded is described in the official [Power BI Embedded site](https://www.azure.cn/home/features/power-bi-embedded/), but let's take a quick look before we get into the details about using it with REST.
...
> [!NOTE]
> We can also provision the workspace collection and get access key via REST API. To learn more, see [Power BI Resource Provider APIs](https://msdn.microsoft.com/zh-cn/library/azure/mt712306.aspx).
...
Now that the provisioning is all done, let’s get started creating a customer’s workspace in the workspace collection via REST APIs. The following HTTP POST Request (REST) is creating the new workspace in our existing workspace collection. This is the [POST Workspace API](https://msdn.microsoft.com/zh-cn/library/azure/mt711503.aspx). In our example, the workspace collection name is **mypbiapp**. 
