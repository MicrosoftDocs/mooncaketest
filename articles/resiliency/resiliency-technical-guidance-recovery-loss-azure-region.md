...
Azure is divided physically and logically into units called regions. A region consists of one or more datacenters that are close.
<!-- regions link reference not suit for Azure.cn-->
<!-- For the current list of regions, see the [Azure regions page](https://azure.microsoft.com/regions/).-->
...
A complete discussion of distributed design is outside the scope of this article. For more information, see [Disaster recovery and high availability for applications built on Azure](https://aka.ms/drtechguide).
