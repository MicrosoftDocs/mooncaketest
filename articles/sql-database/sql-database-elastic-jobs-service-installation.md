...
1. Launch Azure PowerShell command window and navigate to the directory where you downloaded NuGet Command-line Utility (nuget.exe).
...
1. Launch a Azure PowerShell command window and navigate to the \tools sub-directory under the Microsoft.Azure.SqlDatabase.Jobs.x.x.xxx.x folder: Type cd \tools
...
1. From the dashboard view of the elastic database pool via the [Azure Portal](https://portal.azure.cn/#) , click **Create job**.
