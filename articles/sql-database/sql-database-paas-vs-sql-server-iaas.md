...
|**Business continuity:**|In addition to built-in fault tolerance infrastructure capabilities, Azure SQL Database provides features, such as [automated backups](./sql-database-automated-backups.md), [Point-In-Time Restore](./sql-database-recovery-using-backups.md#point-in-time-restore), [Geo-Restore](./sql-database-recovery-using-backups.md#geo-restore), and [Active Geo-Replication](/documentation/articles/sql-database-active-geo-replication/) to increase business continuity. For more information, see [SQL Database business continuity overview](./sql-database-business-continuity.md).|SQL Server on Azure VMs lets you set up a high availability and disaster recovery solution for your database’s specific needs. Therefore, you can have a system that is highly optimized for your application. You can test and run failovers by yourself when needed. For more information, see [High Availability and Disaster Recovery for SQL Server on Azure Virtual Machines](../virtual-machines/windows/sql/virtual-machines-windows-sql-high-availability-dr.md).|
