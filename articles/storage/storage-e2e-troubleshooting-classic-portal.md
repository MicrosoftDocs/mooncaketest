...
- **Azure Management Portal**. You can configure metrics and logging for your storage account in the [Azure Management Portal](https://manage.windowsazure.cn). You can also view charts and graphs that show how your application is performing over time, and configure alerts to notify you if your application performs differently than expected for a specified metric. 

    See [Monitor a storage account in the Azure Portal](./storage-monitor-storage-account.md) for information about configuring monitoring in the Azure Management Portal.
...
Note that the lists below are far from complete. See [Status and Error Codes](http://msdn.microsoft.com/zh-cn/library/azure/dd179382.aspx) for details about general Azure Storage errors and about errors specific to each of the storage services.
...
First, we'll need to configure Azure Storage logging and metrics, so that we have data from the client application to analyze. You can configure logging and metrics in a variety of ways - via the [Azure Management Portal](https://manage.windowsazure.cn), by using PowerShell, or programmatically. See [Enabling Storage Metrics and Viewing Metrics Data](http://msdn.microsoft.com/zh-cn/library/azure/dn782843.aspx) and [Enabling Storage Logging and Accessing Log Data](http://msdn.microsoft.com/zh-cn/library/azure/dn782840.aspx) for details about configuring logging and metrics.
...
> [!NOTE]
> It's not possible to set minute metrics using the Azure Management Portal. However, we recommend that you do set them for the purposes of this tutorial, and for investigating performance issues with your application. You can set minute metrics using PowerShell as shown below, or programmatically, or via the Azure Management Portal.
>
...
To configure client-side logging for a .NET application, enable .NET diagnostics in the application's configuration file (web.config or app.config). See [Client-side Logging with the .NET Storage Client Library](http://msdn.microsoft.com/zh-cn/library/azure/dn782839.aspx) and [Client-side Logging with the Microsoft Azure Storage SDK for Java](http://msdn.microsoft.com/zh-cn/library/azure/dn782844.aspx) for details.
...
Once your application has been running for a period of time, you can review the metrics charts that appear in the Azure Management Portal to observe how your service has been performing. First, we'll add the **Success Percentage** metric to the Monitoring page:

1. Navigate to the Dashboard for your storage account in the [Azure Management Portal](https://manage.windowsazure.cn), then select **Monitor** to view the monitoring page.
...
> [!NOTE]
> It may take some time for your metrics data to appear in the Azure Management Portal after you enable storage metrics. This is because hourly metrics for the previous hour are not displayed in the Azure Management Portal until the current hour has elapsed. Also, minute metrics are not displayed in the Azure Management Portal. So depending on when you enable metrics, it may take up to two hours to see metrics data.
...
For additional information about downloading server-side logs, see [Downloading Storage Logging log data](http://msdn.microsoft.com/zh-cn/library/azure/dn782840.aspx#DownloadingStorageLogginglogdata). 
