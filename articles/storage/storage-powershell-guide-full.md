---
title: Using Azure PowerShell with Azure Storage | Azure
...
In this guide, we’ll explore how to use the [Azure Storage Cmdlets](https://msdn.microsoft.com/zh-cn/library/azure/mt269418.aspx) to perform a variety of development and administration tasks with Azure Storage.

This guide assumes that you have prior experience using [Azure Storage](./index.md) and [Windows PowerShell](http://technet.microsoft.com/zh-cn/library/bb978526.aspx). The guide provides a number of scripts to demonstrate the usage of PowerShell with Azure Storage. You should update the script variables based on your configuration before running each script.
...
**New to Azure:** Get a Azure subscription and a Microsoft account associated with that subscription. For information on Azure purchase options, see [Free Trial](https://www.azure.cn/pricing/1rmb-trial/).

See [Assigning administrator roles in Azure Active Directory (Azure AD)](https://msdn.microsoft.com/zh-cn/library/azure/hh531793.aspx) for more information about Azure subscriptions.

**After creating an Azure subscription and account:**
...
> [!IMPORTANT]
> The name of your storage account must be unique within Azure and must be lowercase. For naming conventions and restrictions, see [About Azure Storage Accounts](./storage-create-storage-account.md) and [Naming and Referencing Containers, Blobs, and Metadata](http://msdn.microsoft.com/zh-cn/library/azure/dd135715.aspx).
...
- Generate a shared access signature token for an Azure storage container and use it to create a storage context:
    ...
    For more information, see [New-AzureStorageContainerSASToken](http://msdn.microsoft.com/zh-cn/library/azure/dn806416.aspx) and [Using Shared Access Signatures (SAS)](./storage-dotnet-shared-access-signature-part-1.md).
...
Azure File storage offers shared storage for applications using the standard SMB protocol. Azure virtual machines and cloud services can share file data across application components via mounted shares, and on-premises applications can access file data in a share via the File storage API or Azure PowerShell.

For more information on Azure File storage, see [Get started with Azure File storage on Windows](./storage-dotnet-how-to-use-files.md) and [File Service REST API](http://msdn.microsoft.com/zh-cn/library/azure/dn167006.aspx).
...
For detailed information on using Storage Metrics and Storage Logging to troubleshoot storage issues, see [Monitoring, Diagnosing, and Troubleshooting Azure Storage](./storage-monitoring-diagnosing-troubleshooting.md).
...
## How to use Azure Storage for Azure China
An Azure environment is an independent deployment of Azure, such as [AzureChinaCloud for Azure operated by 21Vianet in China](http://www.windowsazure.cn/). You can deploy new Azure environments for Azure China.
...
- [Overview of Differences When Creating an Application on China Service](https://msdn.microsoft.com/zh-cn/library/azure/dn578439.aspx)
