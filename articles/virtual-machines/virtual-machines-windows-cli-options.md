...
The Azure Command Line Interface (CLI) provides a command line and scripting environment for creating and managing Azure resources. The Azure CLI is available for Mac OS X, Linux, and Windows operating systems. Across these operating systems, the CLI commands are identical, however operating system specific scripting syntax can differ.

This document details the ways that the Azure CLI can be install and run on Windows and details syntactical considerations for each. For in depth Azure CLI documentation see, [Azure CLI documentation]( https://docs.microsoft.com/cli/azure/overview).

## Windows Subsystem for Linux

The Windows Subsystem for Linux (WSL) provides an Ubuntu Linux environment on Windows 10 Anniversary edition. When enabled, WSL provides a native bash experience, which can be used for creating and running Azure CLI scripts. Because WSL provides a native Bash experience, Azure CLI scripts can be shared between Mac OS X, Linux, and Windows without modification.
...
When using Docker for Windows, a Docker image can be started that includes the Azure CLI. This image is based off of Linux, and includes a native Bash experience.  When using Docker for Windows and the Azure CLI image, scripts to be shared between Mac OS X, Linux, and Windows. 
