---
title: Azure Virtual Machines high availability for SAP NetWeaver | Azure
...
For more information about protecting SAP components in high-availability scenarios, see [Azure Virtual Machines planning and implementation for SAP NetWeaver][planning-guide].
...
- In the Azure portal preview, on the **Parameters** blade, in the **SYSTEMAVAILABILITY** box, select **HA**.
...
1.  In the Azure portal preview, on the **Parameters** blade, in the **NEWOREXISTINGSUBNET** box, select **existing**.
...
- In the Azure portal preview, on the **Parameters** blade, in the **NEWOREXISTINGSUBNET** box, select **new**. Leave the **SUBNETID** field empty.
...
1.  In the Azure portal preview, on the **DNS servers** blade, make sure that your virtual network **DNS servers** option is set to **Custom DNS**.
...
1.  In the Azure portal preview, select **Resource Group** > **Network Card** > **Settings** > **IP Address**.
...
1.  The initial deployment sets the internal load balancer IP address to **Dynamic**. In the Azure portal preview, on the **IP addresses** blade, under **Assignment**, select **Static**.
...
1.  In the Azure portal preview, select **<*SID*>-lb-ascs load balancer** > **Load Balancing Rules**.
...
3.  Restart cluster node A from the Azure portal preview (this initiates an automatic failover of the SAP <*SID*> cluster group from node A to node B).  
