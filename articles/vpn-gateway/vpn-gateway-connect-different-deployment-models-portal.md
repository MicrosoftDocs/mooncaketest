...
In this article, we will use the Azure portal preview and PowerShell. PowerShell is required to create the connections between the virtual networks. You cannot create the connections for this configuration by using the portal.
...
* You have installed the latest PowerShell cmdlets. See [How to install and configure Azure PowerShell](https://docs.microsoft.com/powershell/azureps-cmdlets-docs) for more information. Make sure you install both the Service Management (SM) and the Resource Manager (RM) cmdlets. 
...
In this section, we will create the local network (local site) and the virtual network gateway for your classic VNet. The instructions in this section use the Azure portal preview. The steps here assume that a VPN gateway has not been created for the classic VNet. If you already have a gateway, verify that it is a Dynamic gateway. If it's Static, you must first delete the VPN gateway, then proceed with the following steps.
...
Sometimes the names for classic VNets and Local network sites are changed in the network configuration file when creating classic VNet settings in the Azure portal preview due to the differences in the deployment models. For example, using the Azure portal preview, we named the classic VNet 'Classic VNet' and created it in a resource group named 'ClassicRG'. The name that is contained in the network configuration file is converted to 'Group ClassicRG Classic VNet'. When specifying the name of a VNet that contains spaces, use quotation marks around the value.
...
You can verify your connections by using the Azure portal preview or PowerShell. When verifying, you may need to wait a minute or two as the connection is being created. When a connection is successful, the connectivity state will go from 'Connecting' to 'Connected'.
